...
export async function renderProblema(container, piantaId, problemaId) {
  ...
  container.innerHTML = `
    ...
    <div class="chat-azioni" id="chat-azioni" hidden>
      <button type="button" class="btn btn-secondario" id="btn-chiedi-diagnosi" title="Basta domande: l'AI tira le somme con quello che sa">Tiriamo le somme</button>
    </div>
  ...
  document.getElementById('problema-stato').addEventListener('change', async (evento) => {
    ...
    try {
      ...
      if (vecchioStato === 'in_corso' && nuovoStato !== 'in_corso' && stato.messaggi.some((m) => m.ruolo === 'utente')) {
        try {
          await generaRiassunto();
        } catch {
          // generaRiassunto mostra già il toast di errore; qui serve solo
          // che il bottone "Aggiorna riassunto" appaia per dare un via di recupero.
          disegnaRiassunto();
        }
      }
    } catch (errore) {
      mostraErrore('Non sono riuscito a cambiare lo stato: ' + errore.message);
  ...
  document.getElementById('btn-elimina-problema').addEventListener('click', async () => {
    ...
    try {
      ...
      mostraErrore("Non sono riuscito a eliminarla: " + errore.message);
    }
  });
...
function disegnaRiassunto() {
  ...
  if (!r) {
    // Se la conversazione è chiusa (non in_corso) e ha messaggi ma nessun
    // riassunto, il riassunto è mancato: mostra il bottone di recupero.
    const chiusoSenzaRiassunto =
      stato.problema.stato !== 'in_corso' && stato.messaggi.some((m) => m.ruolo === 'utente');
    if (chiusoSenzaRiassunto) {
      contenitore.innerHTML = `
        <div class="riassunto">
          <p class="placeholder" style="margin:0 0 0.4rem;">Il riassunto non è stato generato.</p>
          <button class="btn btn-secondario" id="btn-aggiorna-riassunto" style="padding:0.4rem 0.8rem; font-size:0.85rem;">Aggiorna riassunto</button>
        </div>`;
      document.getElementById('btn-aggiorna-riassunto').addEventListener('click', generaRiassunto);
    } else {
      contenitore.innerHTML = '';
    }
    return;
...
function disegnaConversazione() {
  ...
  if (stato.messaggi.length === 0 && !stato.elaborando) {
    contenitore.innerHTML = `<p class="placeholder">Racconta cosa hai notato: partiamo da lì.</p>`;
    return;
  }
...
function modificaTitolo() {
  ...
  const salva = async () => {
    ...
    try {
      ...
      mostraErrore('Non sono riuscito a salvare il titolo: ' + errore.message);
    }
...
async function inviaTesto(testo, fotoDaInviare) {
  ...
  // Cattura gli id al momento dell'invio: se l'utente naviga via prima che
  // la risposta AI arrivi, stato.piantaId/problemaId saranno già cambiati.
  const piantaId = stato.piantaId;
  const problemaId = stato.problemaId;
  const pianta = stato.pianta;
  const problemiAltri = stato.problemiAltri;
  const problema = stato.problema;

  try {
    await aggiungiMessaggio(piantaId, problemaId, { ruolo: 'utente', testo, fotoB64: fotoDaInviare });
  } catch (errore) {
    mostraErrore('Non sono riuscito a inviare il messaggio: ' + errore.message);
    return;
  }

  // Segna "elaborando" solo se siamo ancora su questa conversazione.
  const stessoContesto = () => stato.piantaId === piantaId && stato.problemaId === problemaId;
  ...
  try {
    const riaperto = !!problema.riassunto;
    const contesto = costruisciContesto(pianta, problemiAltri, problema, riaperto);
    // Cattura i messaggi correnti: se l'utente ha già navigato via, stato.messaggi
    // appartiene a un'altra conversazione — in quel caso non ha senso chiamare l'AI.
    if (!stessoContesto()) return;
    const messaggiApi = [
    ...
    // Salva sulla conversazione originale, non su quella eventualmente aperta dopo.
    await aggiungiMessaggio(piantaId, problemaId, { ruolo: 'ai', testo: rispostaAi });
  } catch (errore) {
...
async function generaRiassunto() {
  ...
  // Cattura gli id al momento della chiamata (stessa logica di inviaTesto).
  const piantaId = stato.piantaId;
  const problemaId = stato.problemaId;
  ...
  try {
    ...
    await aggiornaProblema(piantaId, problemaId, aggiornamenti);
    // Aggiorna lo stato locale solo se siamo ancora sulla stessa conversazione.
    if (stato.piantaId === piantaId && stato.problemaId === problemaId) {
    ...
    mostraErrore('Non sono riuscito ad aggiornare il riassunto: ' + errore.message);
  }
}
